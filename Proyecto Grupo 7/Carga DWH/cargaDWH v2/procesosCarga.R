...
cargarTiempo <- function(){
  ...
  for(i in c(1994:2020)){ #Rango de Años de la Dimensión Tiempo
...
cargarTablaDeHechos <- function() {
  ...
  idHechoCrim <- 0L
  totalComun <- length(comunidades$Comunidad)
  for(anio in c(1995:2020)){ #Recorremos los distintos años
    
    for(mes in c(1:12)){
      periodo <- ""
      if(mes < 10){
        periodo <- paste(anio,mes,sep = "M0")  
      }else{
        periodo <- paste(anio,mes,sep = "M")  
      }
      ...
      for(i_comunidadAct in c(1:totalComun)){
        
        #comunidad <- comunidades$Comunidad[i_comunidadAct]
        comunidad <- as.character(comunidades$Comunidad[i_comunidadAct])
        
        turisAct <- turismo[which(turismo$Periodo==periodo & turismo$Tipo.de.dato=="Dato base" & turismo$X.U.FEFF.Comunidades.autónomas==comunidad),]
        
        idTurismo <- turisAct$Total
        if(length(idTurismo) == 0L){
          idTurismo <- "NULL"
        }
        ...
        sql <- sprintf("SELECT * FROM `comunidad` WHERE `comunidad`.`NombreComunidad`='%s'", comunidades$Comunidad[i_comunidadAct]) #comunidades$Comunidad[3]
